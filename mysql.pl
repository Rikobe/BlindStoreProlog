...
esPadre('Medicina',X):-
    abrir_conexion,
    odbc_query('swiprolog',
               'SELECT nombre FROM id_producto WHERE IdProducto = 15',row(X)),
    cerrar_conexion.
esPadre('Medicina',X):-
    abrir_conexion,
    odbc_query('swiprolog',
               'SELECT nombre FROM id_producto WHERE IdProducto = 21',row(X)),
    cerrar_conexion.



arbol(['Inicio'|[Arbol]]):-
    insertarElemento(Arbol,'Pasillo 1').


insertarElemento([Arbol],Elemento):-
    nth0('Inicio',Arbol,Elemento).
